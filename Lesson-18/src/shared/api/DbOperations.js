...
class DbOperations {
  ...
  // get favorites array for user_id
  async getFilledFavoritesByUserId(userId) {
    const snap = await getDoc(doc(this.collectionRef, userId));
    if (!snap.exists()) return [];

    const { favoritesList: favoritesListIds } = snap.data();

    const filledFavoritesItems = await Promise.all(
      favoritesListIds.map(async (id) => {
  ...
  async getFavoritesByUserId(userId) {
    const snap = await getDoc(doc(this.collectionRef, userId));
    if (!snap.exists()) return [];
    return snap.data(); // { favoritesList: [ ... ] }
  }
